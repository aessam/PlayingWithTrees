...
class Node():
    ...
    def fix_heights(self):
        ...
        steps = 0
        while parent:
            parent.height = steps if steps > parent.height else parent.height
    ...
    def go_through_all(self):
        ...
        while nextItem:
            ...
            if nextItem and nextItem.left:
                ...
                pairs.append((nextItem.value, nextItem.left.value, "L"))
                # counter += 1
                # self.__generate_snapshot(pairs, counter)
            if nextItem and nextItem.right:
                navi.append(nextItem.right)
                pairs.append((nextItem.value, nextItem.right.value, "R"))
                # counter += 1
                # self.__generate_snapshot(pairs, counter)
